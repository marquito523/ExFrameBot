...
module.exports = {
    ...
    description: 'n',
     execute(client, message, args) {
        ...
        let MemberWarns = db.get(`guild_${message.guild.id}_member_${message.author.id}`) || 0


        MemberWarns = MemberWarns * 1
        ...
        mentioned.send(warningEmbed); // DMs the user the above embed!
            ...
            try{
             ...
             db.set(`guild_${message.guild.id}_member_${message.author.id}`, MemberWarns + 1)

             let N = db.get(`guild_${message.guild.id}_member_${message.author.id}`)

             message.channel.send(new Discord.MessageEmbed().setDescription(`:white_check_mark: User now has ${N} warns`).setTitle("Succes"))
            
            }catch(e){

                return message.channel.send(" :x: Unexpected error!")
