...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (user) {
        ...
        if(user.id === message.author.id)return message.channel.send(new Discord.MessageEmbed().setDescription(`Why would you want to ban yourself..? No, you can't do that. I don't want to ban a awesome person like you!`).setTitle("Banning Yourself").setFooter("Come on, we need you in this server."))
        ...
        if(!member) return message.channel.send((new Discord.MessageEmbed().setDescription(`:x: The user you are trying to ban isn't within this guild.`)))
            ...
            if (member) {
                ...
                member.ban({
                    ...
                    var embed = new Discord.MessageEmbed()
                        .setTimestamp()
                        .setDescription(`:x: You do not have permission to ban that user. Please check his [permissions](https://support.discord.com/hc/en-us/articles/206029707-How-do-I-set-up-Permissions-).`)
                        .setFooter("ExFrame ban pannel, Developped by marquito523 ");
