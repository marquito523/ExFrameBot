...
module.exports = {
    ...
    async execute(client, message, args) {
        return 
        if (message.content === `${client.prefix}channel`) {
            ...
            if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(new MessageEmbed().setDescription("You do not have permission to perform that action!"));
            ...
            if (channel.id === settings.SuggestionChannel.id) {
                ...
                try {
                    if(channel.id === message.channel.id)return message.channel.send(new MessageEmbed().setDescription(`You can no longer delete the channel in which your sending messages to avoid any types of bugs. This feature may sound useless, but we need to prevent any types of bugs before they occire. We hope you understand.`).setTitle("Oh no!"))
                    channel.delete()
                    if (!message.channel) return
                    setTimeout(() => {
                        if (!message.channel) return
        ...
        }else if(message.content.startsWith(`${client.prefix}channel shutdown`)){
            const channel = message.mentions.channels.first();
            if(!channel)return message.channel.send(new MessageEmbed().setDescription(`You have not specified any channels to shut down!`));

            const varmessage = await message.channel.send(new MessageEmbed().setDescription("For how long would you like to shut down the channel?"))
